...
| Combat entry, readiness, action selection, and single-target selection | Script + Gated Step | W1_MQ01 -> Scrap Yard -> Faulted Loader | Yes | Pause ATB during required responses. Guide Nova -> Attack -> Faulted Loader before introducing Abilities, weaknesses, Stability Break, or cooldowns. |
| Timed Guard | Contextual timed prompt | First incoming enemy attack | Yes | The first Guard window is generous and explains that tapping reduces incoming damage. Later attacks use the standard timing window without repeating tutorial copy. |
| Guard Break / Stagger as the primary "puzzle key" | Practice + Feedback + Gated Step | W1_SQ03 "Heavy Lifting" -> Security Post -> Acoustic Bulwark certification | Yes (soft gate) | The guided basic attack is guaranteed to connect but deals **0 damage** into Guard. Pause on the blocked-hit explanation, then require Hydraulic Kick and confirm Guard Broken before combat resumes. The Loading Dock is cargo setup only and does not contain a second trainer. |
...
| Combat entry/readiness reinforcement | Implicit UI | W1_MQ03 "The Echo" -> Node 2B: Deep Mine -> first mandatory fight trigger | Yes | The Faulted Loader owns the explicit interface tutorial. Keep this encounter moving; do not stack another full prompt. |
...
| Elemental weakness (+ tempo/stagger payoff) | Practice + Feedback | W1_MQ03 -> first enemy with clear weakness (e.g., Drone weak to Shock) | Yes | Implemented as a compact combat banner: `Weakness hit` / `Cooldowns reduced`, tagged `Cooldown -1`. |
| Status effects (DoT, stun, blind) basics | Practice + Feedback | W1_SQ02 (if completed) OR W1_MQ03 second combat | Optional fallback | Keep World 1 reactive/brute-force only. Use status chips/log feedback unless playtests show confusion. Do not tutorial **Jammed** or **Marked** until World 2 (Zeke joins too late). |
