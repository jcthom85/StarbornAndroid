...
- `start_new_game.yaml`: clears app state, plays the mixed narration/dialogue Sector 9 prologue, and verifies exploration opens in Nova's Bunk. Voiced cinematic cards use timed card taps because only narration cards expose the `Tap to continue` accessibility label.
- `mainquest_wake_up_call.yaml`: starts a fresh game and verifies the full `Wake Up Call` route through quota dialogue, bunk inspection, Jed dialogue, the loader diagnostic and Shock weakness, Functional Cryo-Inductor crafting, Flux Liner grounding, explicit governor confirmation, live cutter test, and fault record. The flow dismisses item toasts, event messages, and dialogue as separate UI layers.
...
- `mainquest_the_echo.yaml`: starts from the valid Deep Mine resume checkpoint, verifies Boggs' MQ03 assignment, cleared-encounter suppression, and Echo Chamber navigation, enters 87 kHz / 68% / 180 degrees through the accessible tuning value fields, then verifies the handshake consequences, Fork grant, Blast Wave tutorial, and `Red Alert` handoff.
- `mainquest_world2_anchor_drill.yaml`: starts from the Hunter Canopy checkpoint, verifies confrontation exposes the Beast without Link, defeats the real Source Beast with normal combat commands, and verifies Beast victory → Anchor Drill → Link → `Liftoff` ordering.
