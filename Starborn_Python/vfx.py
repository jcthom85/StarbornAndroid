...
from kivy.graphics import Color, Rectangle, Ellipse, StencilPush, StencilUse, StencilPop, PushMatrix, PopMatrix, Rotate
...
class VFX:
    ...
    def play_final_blow(self, attacker, target, on_complete=None):
        """
        Plays a dramatic, split-screen final blow cinematic.
        - Slows time.
        - Diagonally splits the screen.
        - Zooms in on the attacker and the defeated target independently.
        - Tilts both views for a comic-book effect.
        """
        from kivy.graphics import Translate, Scale, StencilPop, StencilPush, StencilUse
        from ui.background_snapshot import snapshot as ui_snapshot

        # --- 1. Setup and Time Dilation ---
        # Clock.time_scale = 0.05  # REMOVED: No more slow-mo
        duration = 3.0           # Longer duration for more impact

        # Take a snapshot of the entire screen as it is
        tex = ui_snapshot(self.ui, downsample=1)
        if not tex:
            if on_complete: on_complete()
            return

        # --- 2. Create Overlay and Image Placeholders ---
        overlay = FloatLayout(size=Window.size, pos=(0, 0), opacity=0.0)
        self.ui.add_widget(overlay)

        # Use Scatter widgets to enable independent scale, rotation, and translation animations.
        attacker_scatter = Scatter(size=Window.size, pos=(0,0), do_rotation=False, do_scale=False, do_translation=False)
        attacker_img = Image(texture=tex, size=Window.size, allow_stretch=True, keep_ratio=False)
        attacker_scatter.add_widget(attacker_img)
        
        target_scatter = Scatter(size=Window.size, pos=(0,0), do_rotation=False, do_scale=False, do_translation=False)
        target_img = Image(texture=tex, size=Window.size, allow_stretch=True, keep_ratio=False)
        target_scatter.add_widget(target_img)

        # --- 3. Create StencilViews for the diagonal split ---
        # The StencilView clips any children to the shape defined in its canvas.
        split_angle_deg = 25
        
        # Top-left half (for the attacker)
        attacker_stencil = StencilView(size=Window.size, pos=(0,0))
        with attacker_stencil.canvas.before:
            StencilPush()
            w, h = Window.size
            # Define a polygon for the top-left half of the screen
            offset = math.tan(math.radians(split_angle_deg)) * w * 0.5
            points = [0, h, 0, h - offset, w, 0, w, h]
            # The stencil requires a shape to be drawn. We use a polygon.
            from kivy.graphics import Mesh
            Mesh(vertices=[points[0], points[1], 0, 0, points[2], points[3], 0, 0, points[4], points[5], 0, 0, points[6], points[7], 0, 0], indices=[0, 1, 2, 2, 3, 0], mode='triangle_fan')
        with attacker_stencil.canvas:
            StencilUse()
        with attacker_stencil.canvas.after:
            StencilPop()

        # Bottom-right half (for the target)
        target_stencil = StencilView(size=Window.size, pos=(0,0))
        with target_stencil.canvas.before:
            StencilPush()
            w, h = Window.size
            offset = math.tan(math.radians(split_angle_deg)) * w * 0.5
            points = [0, h - offset, 0, 0, w, 0, w, 0] # Corrected points for bottom-right
            from kivy.graphics import Mesh
            Mesh(vertices=[points[0], points[1], 0, 0, points[2], points[3], 0, 0, points[4], points[5], 0, 0, points[6], points[7], 0, 0], indices=[0, 1, 2, 2, 3, 0], mode='triangle_fan')
        with target_stencil.canvas:
            StencilUse()
        with target_stencil.canvas.after:
            StencilPop()

        # Add the images to their respective stencils
        attacker_stencil.add_widget(attacker_scatter)
        target_stencil.add_widget(target_scatter)

        # Add stencils to the overlay
        overlay.add_widget(attacker_stencil)
        overlay.add_widget(target_stencil)

        # --- 4. Define the diagonal split line ---
        # A bright white line to emphasize the split
        split_angle = -split_angle_deg
        cx, cy = Window.center
        split_line = Widget(size=(Window.width * 2.5, dp(6))) # Make it extra long and thick
        with split_line.canvas:
            Color(1, 1, 1, 0.9) # Bright white
            PushMatrix()
            Translate(cx, cy, 0)
            Rotate(angle=split_angle, axis=(0,0,1))
            Translate(-split_line.width/2, -split_line.height/2, 0)
            Rectangle(pos=(0,0), size=split_line.size)
            PopMatrix()
        split_line.opacity = 0.0
        overlay.add_widget(split_line)

        # --- 5. Animate the effect ---
        Animation(opacity=1.0, duration=0.1).start(overlay)
        Animation(opacity=1.0, duration=0.05).start(split_line) # Flash the line in

        # Animate the zoom and pan for both halves
        zoom = 2.2  # More exaggerated zoom
        tilt = 12.0 # More exaggerated tilt

        # Attacker animation
        attacker_sprite = self.ui.battle_screen._party_panels[self.ui.battle_screen.players.index(attacker)].sprite
        ax, ay = attacker_sprite.to_window(*attacker_sprite.center)
        anim_attacker = Animation(
            scale=zoom, rotation=tilt,
            center_x=ax, center_y=ay + dp(40), # Pan up slightly to focus on waist-up
            duration=duration * 0.9, t='out_cubic'
        )
        anim_attacker.start(attacker_scatter)

        # Target animation
        target_sprite = self.ui.battle_screen._enemy_panels[self.ui.battle_screen.enemies.index(target)].sprite
        tx, ty = target_sprite.to_window(*target_sprite.center)
        anim_target = Animation(scale=zoom, rotation=-tilt, center_x=tx, center_y=ty, duration=duration * 0.9, t='out_cubic')
        anim_target.start(target_scatter)

        # --- 6. Cleanup ---
        def _cleanup(*_):
            # Fade out the entire overlay
            anim_out = Animation(opacity=0.0, duration=0.25)
            anim_out.bind(on_complete=lambda *a: self.ui.remove_widget(overlay) if overlay.parent else None)
            anim_out.start(overlay)
            if on_complete:
                on_complete()

        Clock.schedule_once(_cleanup, duration)
    ...
    def caption(self, text: str, duration: float = 1.5, on_complete=None):
        ...
        def _layout(*_):
            ...
            from kivy.core.window import Window
            cx = getattr(self.ui, "center_x", None)
            cy = getattr(self.ui, "center_y", None)
        ...
        def _fade_out(*_):
            ...
            def _rm(*__):
                # --- FIX: Revert to removing from self.ui ---
    ...
    def narration(self,
        ...
        def _layout(*_):
            ...
            cx = getattr(self.ui, "center_x", None)
            cy = getattr(self.ui, "center_y", None)
            ui_height = getattr(self.ui, "height", None)
            if cx is None or cy is None or ui_height is None:
                from kivy.core.window import Window
                cx, cy = Window.center
                ui_height = Window.height
            x = cx - w / 2
            if position == 'top':
                y = ui_height - h - dp(120)
    ...
    def cinematic_text(self,
        ...
        def _layout(*_):
            ...
            w = max_w
            h = lbl.texture_size[1] + pad_y * 2
            # --- FIX: Position relative to Window, then convert to self.ui's local space ---
            win_cx, win_cy = Window.center
            # Target position in window coordinates
            target_x = win_cx - w/2

            if position == 'top':
                target_y = Window.height - h - dp(140)
            elif position == 'bottom':
                target_y = dp(160)
            else:
                target_y = win_cy - h/2
            # Convert window coordinates to local widget coordinates
            x, y = self.ui.to_widget(target_x, target_y, relative=False)
            bar.size = (w, h)
            bar.pos  = (x, y)
            lbl.size = (w - pad_x * 2, h - pad_y * 2)
            lbl.pos  = (pad_x, pad_y)
