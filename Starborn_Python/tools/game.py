...
from kivy.graphics   import Color, Ellipse, Triangle, PushMatrix, PopMatrix, Scale, Translate, Rectangle, Line, RoundedRectangle
...
def go(direction):
    ref_direction = direction.lower().replace(" ", "_")
    # THIS IS THE FIX: No parentheses after 'exits'
    if not hasattr(current_game.current_room, ref_direction) and \
       ref_direction not in current_game.current_room.exits:
    ...
    nxt = current_game.current_room.exit(direction)
    if nxt:
        ...
        dx, dy = current_game.direction_vectors.get(direction.lower(), (0, 0))
...
def use(item_name, target_name):
    ...
    if not target:
        game.say(f"Could not find a party member named {target_name}.")
...
class DirectionMarker(Widget):
    """
    A subtle glowing arrow marker for one cardinal direction.
    Non-interactive hint; pulses opacity softly.
    """
    direction = StringProperty("east")        # 'north'|'east'|'south'|'west'
    active    = BooleanProperty(False)
    base_rgba = ListProperty([0.56, 0.78, 1.0, 0.7])  # starborn cyan-blue w/ alpha
    ...
    def __init__(self, **kwargs):
        ...
        self._anim = None
        self.bind(pos=self._redraw, size=self._redraw, pulse=self._redraw)
        self._redraw()
    ...
    def _start_pulse(self):
        ...
        # fade up & down + expand glow
        self._anim = (
            Animation(opacity=0.95, pulse=1.0, d=0.8)
            + Animation(opacity=0.60, pulse=0.0, d=0.8)
        )
    ...
    def _arrow_points(self):
        # Triangle arrow in local 56x56 box, centered; rotated per direction
        cx, cy = self.center
        w, h = self.size
        # Build an EAST-facing triangle first
        p1 = (cx - dp(10), cy - dp(12))
        p2 = (cx - dp(10), cy + dp(12))
        p3 = (cx + dp(14), cy)
        # We'll rotate by swapping/negating coordinates after drawing via canvas transforms
        return [p1, p2, p3]

    def _redraw(self, *_):
        self.canvas.clear()
        with self.canvas:
            # Soft glow: two ellipses expanding with pulse
            base = min(self.width, self.height) * 0.45
            glow_r1 = base * (1.15 + 0.25 * self.pulse)
            glow_r2 = base * (1.45 + 0.40 * self.pulse)

            # outer glow
            Color(self.base_rgba[0], self.base_rgba[1], self.base_rgba[2], 0.26)
            Ellipse(pos=(self.center_x - glow_r2, self.center_y - glow_r2),
                    size=(glow_r2 * 2, glow_r2 * 2))
            # inner glow
            Color(self.base_rgba[0], self.base_rgba[1], self.base_rgba[2], 0.42)
            Ellipse(pos=(self.center_x - glow_r1, self.center_y - glow_r1),
                    size=(glow_r1 * 2, glow_r1 * 2))

            # Arrow head (triangle)
            Color(self.base_rgba[0], self.base_rgba[1], self.base_rgba[2], 1.0)
            PushMatrix()
            # Translate to center for rotation
            Translate(self.center_x, self.center_y, 0)
            # Rotate by setting scale on swapped axes; simpler than Matrix rotation for cardinal steps
            # Direction mapping:
            # east: identity; north: swap x/y with y->-x; west: x->-x; south: swap x/y with x->-y
            # We'll do it by Scale with signs and then swap coordinates via drawing reference points relative to origin.
            # To keep it simple, just pick a per-direction transform scale.
            if self.direction == "east":
                ...
                # rotate -90 -> swap x/y and flip y
                Scale(0, 0, 1)  # no-op; triangle constructed points will be rebuilt for north/south below
            elif self.direction == "south":
                Scale(0, 0, 1)

            PopMatrix()

            # For clarity and to avoid transform headaches, draw explicit per-direction triangles:
            Color(self.base_rgba[0], self.base_rgba[1], self.base_rgba[2], 1.0)
            cx, cy = self.center
            # Compute triangle points for this direction
            tri = None
            if self.direction == "east":
                tri = [
                    cx - dp(10), cy - dp(12),
            ...
            # Fill
            Triangle(points=tri)
            # Outline for clarity on bright backgrounds
            Color(self.base_rgba[0], self.base_rgba[1], self.base_rgba[2], 1.0)
...
class MainWidget(FloatLayout):
    ...
    def use_item(self, item, target):
        ...
        if item.name not in self.inventory or self.inventory[item.name] <= 0:
            self.say(f"You don't have any {item.name}s.")
            return

        effect = getattr(item, 'effect', {})
        if not effect:
            self.say(f"You can't use the {item.name} right now.")
            return
        ...
        # --- Effect Dispatcher ---
        # NEW: This now correctly uses the buff system from entities.py
        if "buff_stat" in effect:
            buff_info = effect["buff_stat"]
            stat, value, duration = buff_info["stat"], buff_info["value"], buff_info["duration"]
            # This is now a list, so we add it to the new "buffs" structure
            effect.setdefault("buffs", []).append({"stat": stat, "value": value, "duration": duration})

        if "buffs" in effect:
            for buff_info in effect.get("buffs", []):
                stat, value, duration = buff_info["stat"], buff_info["value"], buff_info["duration"]
                temp_id = f"temp_{item.name.lower().replace(' ', '_')}_{stat}"
                temp_def = {
                    "id": temp_id, "duration": duration, "stack": "refresh",
                    "effects": {"flat": {stat: value}, "mult": {}, "resist": {}},
                    "flags": []
                }
                self.buff_index[temp_id] = temp_def
                target.add_buff(temp_id)
                self.say(f"{target.name}'s {stat.upper()} was boosted!")

        if "restore_hp" in effect:
            heal_amount = effect["restore_hp"]
            target.hp = min(target.total_max_hp, target.hp + heal_amount)
            self.say(f"{target.name} recovered {heal_amount} HP.")
            
        if "restore_rp" in effect:
            rp_amount = effect["restore_rp"]
            target.rp = min(getattr(target, 'max_rp', target.rp), target.rp + rp_amount)
            self.say(f"{target.name}'s Resonance was restored by {rp_amount}.")
        
        elif "cure_status" in effect:
            status_to_cure = effect.get("cure_status")
            if status_to_cure == "all":
                target.status_effects.clear()
                self.say(f"{target.name}'s ailments were cleansed.")
            else:
                try:
                    st_enum = Status[status_to_cure.upper()]
                    if st_enum in target.status_effects:
                        del target.status_effects[st_enum]
                        self.say(f"{target.name} is no longer {status_to_cure}.")
                except KeyError:
                    pass # Status not found
        
        # Other effects like damage, utility, etc. would be handled here
        # For now, we just log them if they aren't the primary ones above
        else:
            self.say(f"The {item.name} is ready for use.")
    ...
    def _make_titled_popup(self, title, body, **overrides):
        ...
        x_btn = Button(
            text="X",
            size_hint=(None, None),
            size=(cfg["header_height"], cfg["header_height"]),
    ...
    def __init__(self, **kwargs):
        ...
        self._skill_index: dict | None = None # ADD THIS LINE
        
        # FIX: Initialize visual components here to avoid race condition
    ...
    def _post_init_setup(self, dt):
        ...
        try:
            from kivy.uix.widget import Widget
            from kivy.graphics import Color, Rectangle
            self._world_dark = Widget(size_hint=(None, None))
            with self._world_dark.canvas:
                self._world_dark_col = Color(0, 0, 0, 0)
                self._world_dark_rect = Rectangle(pos=(0, 0), size=(1, 1))
            # Insert just above the background layers
            main_container.add_widget(self._world_dark, index=max(0, len(main_container.children) - 1))
        except Exception:
        ...
        if bands is not None:
            ...
            def _layout_top(*_):
                ...
                # Minimap sizing/placement (unchanged)
                if minimap:
                    mw, mh = minimap.size
                ...
                if title:
                    right_pad = (minimap.width + pad * 2) if minimap else pad
            ...
            def _layout_scroll(*_):
                ...
                sv.height = max(0, title.y - y - inset * 2)
                sv.pos = (0, y + inset)
    ...
    def load_item_and_npc_data(self):
        ...
        for itm_def in _load_list_json("items.json"):
            ...
            # Explicitly handle 'unsellable' for key items
            if item_obj.type == 'key_item':
                setattr(item_obj, 'unsellable', True)
        ...
        for en in _load_list_json("enemies.json"):
            ...
            self.enemy_flavor[en["id"]] = en.get("flavor", en.get("description", ""))
            ...
            if trig:
                ...
                for part in trig.split(","):
                    ...
                    if t_typ == "give_item":
                        ...
                        self.say(f"You receive [b]{t_val}[/b].")
                        ...
                        if character := self.party_defs.get(char_id):
                            if skill_id not in character.unlocked_abilities:
                                ...
                                self.say(f"[color=00ff00][b]{character.name} learned a unique skill: {skill_name}![/b][/color]")
    ...
    def _attempt_move(self, direction):
        ...
        if self.current_room and direction in self.current_room.exits:
            ...
            App.get_running_app().tx_mgr.slide_transition(direction, lambda: go(direction), duration=0.50)
        else:
    ...
    def save_progress(self, slot: int = 1):
        ...
        try:
            ...
            self.say(f"[i]Game saved to slot {slot}.[/i]")
        except Exception as e:
            self.say(f"[color=ff0000]Error saving game: {e}[/color]")
    ...
    def _apply_loaded_state(self, data: dict, slot: int):
        ...
        for room_id, states in rooms_state_data.items():
            if room := self.rooms.get(room_id):
                if not hasattr(room, 'state'):
                    room.state = {}
                room.state.update(states)
    ...
    def _load_game_state(self, slot: int):
        ...
        if not data:
            ...
            self.say(f"[color=ff0000]Failed to load save file. Starting new game.[/color]")
            self._initialize_new_game_state()
            return
    ...
    def narrate(self, text: str, *, title: str | None = None, actions=None, autoclose=True, tap_to_dismiss=True):
        ...
        try:
            ...
            NarrativePopup.show(text, title=title, actions=actions, theme_mgr=self.themes,
    ...
    def update_room_display(self):
        ...
        try:
            ...
            if _app and hasattr(_app, 'set_global_dark'):
        ...
        if "title_label" in self.ids:
            self.ids.title_label.text = self.room_title
            Clock.schedule_once(self._update_title_underline)
        ...
        if not self._room_is_dark:
            ...
            enemy_flavor_map = getattr(room, 'enemy_flavor_map', {})
            for enemy in getattr(room, 'enemies', []):
                # Prefer room-specific flavor, fall back to enemy's own description.
                flavor = enemy_flavor_map.get(enemy.enemy_id)
                if not flavor:
                    flavor = self.enemy_flavor.get(enemy.enemy_id)
                if flavor:
                    flavor_texts.append(str(flavor).strip())

        # 3) Combine base description and all flavor texts into a single block.
        ...
        self.room_details_text = self.inject_clickables(full_text_block, room)
    ...
    def update_direction_markers(self):
        ...
        show = self._unexplored_adjacent_directions()
        ...
        for d, marker in self._dir_markers.items():
            marker.active = (d in show)
            # Visibility handled by the marker's on_active (pulsing). If inactive, it fades to 0.
            marker.opacity = 1.0 if marker.active else 0.0
    ...
    def _enemy_go_alert(self, enemy_item, *_):
        ...
        self.say(f"The {enemy_item.name} focuses its sensors on you???")
        ev = Clock.schedule_once(
    ...
    def _enemy_start_ambush(self, enemy_item, *_):
        ...
        self.say(f"[color=ff7f50][b]{enemy_item.name} attacks![/b][/color]")
        self.start_battle(
            ...
            if act_key == "storage locker":
                if self.inventory.get("a small key", 0) > 0:
                    def _unlock():
                        ...
                        self.say("You use the small key. The locker clicks open.")
                        self.update_room()
                    self.show_action_popup("Storage Locker", [("Use small key", _unlock)])
                else:
                    self.say(room_act.get("condition_unmet_message", "It's locked tight."))
                return True # <-- ADD THIS LINE

            atype = room_act.get("type")
            if atype == "toggle":
                state_key = room_act.get("state_key")
                event_name = room_act.get("action_event")
                if not state_key or not event_name:
                    self.say("(Broken toggle action.)")
                    return True # <-- ADD THIS LINE
        ...
        if ref.startswith("attack_"):
            ...
            if not enemy_item:
                ...
                if base_enemy:
                    ...
                    enemy_item = self.current_room.enemies.find(eid)
                if not enemy_item:
                    self.say("[i](That enemy is already defeated.)[/i]")
                    return True # <-- ADD THIS LINE
    ...
    def handle_npc_interaction(self, npc, interaction_def):
        ...
        if itype == "talk" or itype == "quest_talk":
            ...
            if dialogue_id:
                ...
                self.say(f"{npc.name} has nothing to say right now.")
            ...
            if shop_id:
                ...
                self.say(f"{npc.name} doesn't seem to be selling anything.")
            ...
            if not item_obj:
                 self.say(f"(DEBUG: Unknown item '{item_name_to_give}' in interaction def)")
                 return
            ...
            if self.inventory.get(item_obj.name, 0) > 0:
                ...
                self.say(f"You don't have a {item_obj.name} to give.")
        
        elif itype == "recruit":
            self.recruit(npc.name.lower())
        
        else:
            self.say(f"(Unhandled interaction type: {itype})")

    def use_item(self, item, target):
        """Applies a consumable item's effect."""
        if item.name not in self.inventory or self.inventory[item.name] <= 0:
            self.say(f"You don't have any {item.name}s.")
            return
        ...
        if effect.get("type") == "heal":
            ...
            self.say(f"{target.name} recovered {amount} HP.")
        # Add other effect types like 'damage' for combat later
        ...
        if new_level > old_level:
            ...
            for level_gained in range(old_level + 1, new_level + 1):
                ...
                if skill_to_learn and skill_to_learn not in character.unlocked_abilities:
                    ...
                    self.say(f"[color=00ff00][b]{character.name} learned {skill_name}![/b][/color]")

            self.say(f"[b]{character.name} reached Level {new_level}![/b]")
    ...
    def start_battle(self,
        ...
        def switch_to_battle_screen(final_bg_texture):
            ...
            def _on_defeat():
                self.say("[i]The party wakes up back in the living quarters???[/i]")
                for p in players:
                    p.hp = p.max_hp

            # Reset resonance to the starting value before battle begins.
            ...
            self.milestones.on_battle_end(enemy_ids_list[0])
    ...
    def start_shop(self, shop_id: str):
        ...
        if shop_id not in self.all_shops:
            self.say("This shop is not open right now.")
            return
...
class StarbornApp(App):
    ...
    def set_global_dark(self, alpha: float, *, animate: bool = False, duration: float = 0.25):
        try:
            ...
            if not hasattr(self, '_dark_col') or self._dark_col is None:
                return
            if animate:
                Animation.cancel_all(self._dark_col)
    ...
    def _start_game(self, slot):
        ...
        if main_widget.current_room:
            # Loaded a save that was inside a room
            # Start fade immediately (before switching screens) to prevent a flash
            self.tx_mgr.fade_in_exploration(1.0)
            self.screen_manager.current = "explore"
        else:
            # New game or hub save: play intro, then start in Nova's House (Rusthaven)
            def _enter_start_room_after_intro(*_):
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    # Start the long fade immediately (before switching screens)
                    # to ensure the black scrim is in place and there is no flash.
                    ...
                    AudioEvents.emit("room.enter", {
                        "hub_id": wm.current_hub_id,
                        "room_id": None,
                    })

            def _enter_hub_after_intro(*_):
                # Used when resuming a hub save (not a brand-new game)
            ...
            if slot is None:
                ...
                try:
                    ...
                    _enter_start_room_after_intro()
            else:
                _enter_hub_after_intro()
    ...
    def load_settings(self):
        try:
            ...
            self.settings = {'screenshake': False, 'flashes': False, 'haptics': False}
