...
3) No other edits required. The dialog will read data from your editor and apply changes in‑memory,
   then mark the project dirty so Save All writes nodes.json/rooms.json.
...
# that the assistant can diff/preview before writing rooms.json/nodes.json.
...
def _pg_find_root(start: Path) -> Path:
    cur = start.resolve()
    for _ in range(6):
        if (cur / "rooms.json").exists() or (cur / "nodes.json").exists():
            return cur
        cur = cur.parent
...
def compute_cluster_payload(project_root: str | Path,
    ...
    The caller (AI Assistant) will merge + diff these into rooms.json and nodes.json.
    """
    ...
    if not node:
        raise ValueError(f"Node '{node_id}' not found in nodes.json.")
