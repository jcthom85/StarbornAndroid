...
â€¢ Multi-select pickers for Room Items & NPCs (with search filter).
â€¢ Connection pickers & completers (choose valid rooms quickly).
â€¢ Project-wide Validate (catch broken refs, orphans, bad ranges).
â€¢ Snap-to-grid toggle for room layout.
â€¢ Tree search filter (find worlds/hubs/nodes/rooms fast).
â€¢ Ctrl+S to Save All, visual "dirty" dot in title.
â€¢ PNG snapshot export of the current canvas (hub or node mode).
â€¢ Safer renaming with ID remapping where relevant.
â€¢ Numerous UX refinements.
...
from PyQt5.QtCore import Qt, QPointF, QRectF, pyqtSignal, QEvent, QSize, QMimeData, QSizeF
from PyQt5.QtGui import (
    QPainter, QPen, QBrush, QColor, QCursor, QPixmap, QFontMetrics, QImage,
    QDrag, QIcon, QKeySequence
)
...
class EditEnemyDialog(JsonEditorDialog):
    def __init__(self, all_enemies: List[str], parent=None):
        ...
        self.add_field("party", "Party (CSV):", QLineEdit())
...
class EditActionDialog(JsonEditorDialog):
    def __init__(self, parent=None):
        ...
        type_combo.addItems(["", "tinkering", "cooking", "fishing", "toggle", "shop", "player_action"])
        self.add_field("type", "Type:", type_combo)
        self.add_field("state_key", "State Key (toggle):", QLineEdit())
...
class VisualCanvas(QWidget):
    ...
    def set_mode(self, mode: int, hub_id: Optional[str] = None, visible_room_ids: Optional[Set[str]] = None):
        ...
        self._bg_pix = None
        # Always use the logical scene size for hub math (matches game)
        ...
        if self.mode == self.MODE_HUB and self.current_hub_id:
            ...
            if hub and hub.get('background_image'):
                ...
                if os.path.exists(path):
                    pix = QPixmap(path)
                    if not pix.isNull():
                        self._bg_pix = pix  # keep actual pixels; draw it stretched to the scene later
    ...
    def _get_node_rect_scene(self, node: dict) -> QRectF:
        ...
        sz_w, sz_h = node.get("size", [128, 128])
    ...
    def _paint_hub_mode(self, p: QPainter):
        ...
        if self._bg_pix:
            ...
            p.drawPixmap(scene_rect, self._bg_pix, src)  # stretch bg to 9:21 scene
        else:
        ...
        for nid, node in self.nodes_by_id.items():
            ...
            # Title strip overlay for readability
            title = node.get("title", nid)
            label_h = max(20.0, min(28.0, rect.height() * 0.22))
            label_rect = QRectF(rect.left(), rect.bottom() - label_h, rect.width(), label_h)
            p.fillRect(label_rect, QColor(0, 0, 0, 110))
            p.setPen(Qt.white)
            p.drawText(label_rect.adjusted(6, 0, -6, 0), Qt.AlignVCenter | Qt.AlignLeft, title)
    ...
    def _mousePressHub(self, ev):
        ...
        if nid:
            ...
            if ev.button() == Qt.LeftButton:
                ...
                self.node_selected.emit(nid)  # <- this triggers tree selection & can switch to Node mode
                if handle:
                    # resize only on left
                ...
                # NOTE: we intentionally do NOT start a node drag on left-click,
                # because the selection will flip the editor into Node mode.
            else:
    ...
    def _mouseMoveHub(self, ev):
        ...
        if self._dragging_node and self.selected_node_id:
            ...
            node = self.nodes_by_id[self.selected_node_id]
            dx = p_now.x() - self._drag_start_scene.x(); dy = p_now.y() - self._drag_start_scene.y()
            rect = QRectF(self._drag_start_node_center - self._drag_start_node_size/2, self._drag_start_node_size)
            if self._resize_handle == "tl": rect.setTopLeft(rect.topLeft() + QPointF(dx, dy))
            elif self._resize_handle == "tr": rect.setTopRight(rect.topRight() + QPointF(dx, dy))
            elif self._resize_handle == "bl": rect.setBottomLeft(rect.bottomLeft() + QPointF(dx, dy))
            elif self._resize_handle == "br": rect.setBottomRight(rect.bottomRight() + QPointF(dx, dy))
            node["size"] = [max(16, int(rect.width())), max(16, int(rect.height()))]
...
class WorldBuilder(QMainWindow):
    def __init__(self, root_dir: str):
        super().__init__()
        self.project_root = root_dir      # â† add
        self.root = root_dir              # â† add
        ...
        self.canvas.node_selected.connect(lambda nid: self._select_in_tree("node", nid) if nid else None)
    ...
    def _init_inspectors(self):
        ...
        self.inspector_stack.addWidget(self._create_inspector_panel("Node", {"ID": self.n_id, "Title": self.n_title, "Entry Room": self.n_entry, "Icon": icon_layout, "Position": pos_layout, "Size": size_layout, "Discovery": self.n_discovered}))
        ...
        room_inspector_widgets = {
            ...
            "Background": r_bg_layout, "Position": r_pos_layout, "Description": self.desc_tabs,
    ...
    def _save_all_data(self, *, show_toast: bool = True):
        self._apply_inspector_changes()  # ensure pending changes are applied
        ok = True
    ...
    def _on_tree_selection_changed(self):
        ...
        if typ == "world":
            ...
            self.canvas.selected_node_id = _id
    ...
    def _select_in_tree(self, kind: str, _id: str):
        if not _id: return
    ...
    def _sync_room_inspector(self):
        ...
        if not r: self._clear_room_inspector(); return

        # Block signals while populating
        widgets_signals = [
            (self.r_title, 'editingFinished'), (self.r_env, 'editingFinished'),
            (self.r_bg, 'editingFinished'), (self.r_pos_x, 'valueChanged'),
            (self.r_pos_y, 'valueChanged'), (self.desc_edit, 'textChanged'),
        ...
        try: self.r_is_entry.blockSignals(True)
        except Exception: pass
        self.r_is_entry.setChecked(is_start)
        try: self.r_is_entry.blockSignals(False)
        except Exception: pass

        if w is self.r_is_entry:
            getattr(w, _).connect(self._on_toggle_starting_room)
        else:
            getattr(w, _).connect(self._apply_room_inspector_changes)

    def _clear_room_inspector(self):
        self.r_id.setText("â€”"); self.r_title.clear(); self.r_env.clear(); self.r_bg.clear()
        ...
        self.r_items.clear(); self.r_npcs.clear(); self.r_enemies_editor.set_data([]); self.r_actions_editor.set_data([]); self.r_state_editor.set_data({})
    ...
    def _apply_room_inspector_changes(self):
        typ, _id = self.current_selection
        if typ != 'room' or not _id or _id not in self.rooms_by_id: return
        self._set_dirty(True)
        r = self.rooms_by_id[_id]
        r["title"] = self.r_title.text(); r["env"] = self.r_env.text(); r["background_image"] = self.r_bg.text()
    ...
    def get_node_hover_info(self, node_id):
        node = self.nodes_by_id.get(node_id, {})
        key = _node_room_key(node); room_ids = node.get(key, [])
        num_rooms = len(room_ids)
        num_quests = 0  # placeholder
        return f"<b>{node.get('title', node_id)}</b><br>{num_rooms} Rooms<br>{num_quests} Quests"
    ...
    def _browse_for_image(self, line_edit_widget):
        path, _ = QFileDialog.getOpenFileName(self, "Select Image", self.root, "Images (*.png *.jpg *.jpeg)")
        if path:
            try:
