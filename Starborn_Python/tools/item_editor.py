...
def _find_root(start: Optional[Path] = None) -> Path:
    """Find a directory containing items.json, walking up from start/CWD."""
    start = Path(start or __file__).resolve()
    candidates = [
        start.parent,                   # tools/
        start.parent.parent,            # project root
        Path.cwd(),
        Path.cwd().parent,
    ]
    for base in candidates:
        if (base / "items.json").exists():
            return base
    # env overrides
    for key in ("STARBORN_ROOT", "STARBORN_PROJECT_ROOT"):
        v = os.environ.get(key)
        if v and (Path(v) / "items.json").exists():
            return Path(v)
    return Path.cwd()
...
class ItemEditor(QWidget):
    def __init__(self, project_root: Optional[str] = None):
        ...
        self.root = Path(project_root).resolve() if project_root else _find_root()
