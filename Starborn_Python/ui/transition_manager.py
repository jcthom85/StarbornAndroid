...
class TransitionManager:
    def __init__(self):
        ...
        self._vignette = None
        self._icon = None
        # Flags used for orchestration with the App (e.g., first-room fade)
        self.abort_on_light_switch : bool = False
        self._explore_fade_active  : bool = False

    # ------------------------------------------------------------------ #
    # Simple full-screen fade-in reveal for exploration UI
    # ------------------------------------------------------------------ #
    def fade_in_exploration(self, duration: float = 1.0):
        """
        Covers the whole window with a black overlay and fades it out.
        This version waits for one frame to ensure the UI is drawn before fading.
        """
        # If another transition is active, don't stack overlays; bail out.
        if self.is_transitioning:
            return
        
        self.is_transitioning = True
        self._explore_fade_active = True

        # --- THIS IS THE FIX ---
        # We wait one frame before creating the overlay. This gives Kivy
        # time to draw the UI elements that were just added to the screen.
        Clock.schedule_once(lambda dt: self._execute_fade_in(duration), 0)

    def _execute_fade_in(self, duration: float):
        """
        The core logic of the fade-in, executed after a one-frame delay.
        """
        try:
            # Clear any forced black screen from cinematics
            app = App.get_running_app()
            if app:
                if hasattr(app, '_force_black_screen'):
                    app._force_black_screen = False
                if hasattr(app, 'set_global_dark'):
                    app.set_global_dark(0.0, animate=False)

            self.overlay = FloatLayout(size=Window.size, pos=(0, 0))
            scrim = Widget(size=Window.size, pos=(0, 0))
            with scrim.canvas:
                self._fade_col = Color(0, 0, 0, 1.0)
                self._fade_rect = Rectangle(size=scrim.size, pos=scrim.pos)
            scrim.bind(size=lambda i, s: setattr(self._fade_rect, 'size', s),
                       pos=lambda i, p: setattr(self._fade_rect, 'pos',  p))
            self.overlay.add_widget(scrim)
            Window.add_widget(self.overlay)

            # Fade the scrim alpha from 1 -> 0, then remove
            anim = Animation(a=0.0, duration=max(0.0, float(duration)), t='out_quad')
            ...
            def _cleanup(*_):
                ...
                self._explore_fade_active = False
                self.is_transitioning = False

            anim.bind(on_complete=_cleanup)
            anim.start(self._fade_col)
        except Exception as e:
            print(f"Error in _execute_fade_in: {e}")
            self.is_transitioning = False

    # ------------------------------------------------------------------ #
    # Utilities: force-complete the exploration fade immediately
    # ------------------------------------------------------------------ #
    def force_finish_exploration_fade(self):
        """Immediately complete and clean up the current exploration fade.

        Safe to call even if no fade is active.
        """
        try:
            if not self.overlay or not getattr(self, '_fade_col', None):
                return
            # Cancel any running animations on the fade color
            try:
                Animation.cancel_all(self._fade_col)
            except Exception:
                pass
            # Snap to fully transparent and remove overlay
            try:
                self._fade_col.a = 0.0
            except Exception:
                pass
            try:
                if self.overlay and self.overlay.parent:
            ...
            self._explore_fade_active = False
            # --- THIS IS THE FIX ---
            # Also unlock the game input so the player can move immediately.
            self.is_transitioning = False
            try:
                app = App.get_running_app()
                if app and hasattr(app, 'current_game'):
                    app.current_game.input_locked = False
            except Exception: pass
            self.abort_on_light_switch = False
        except Exception:
            # Best-effort cleanup
            try:
                self.overlay = None
    ...
    def fade_in_exploration_after_layout(self, main_widget, duration: float = 1.0):
        """
        Waits for the main_widget to be laid out before covering the screen
        with a black overlay and fading it out.
        """
        # If another transition is active, don't stack overlays; bail out.
        if self.is_transitioning:
            return
        ...
        # Use Clock.schedule_once to wait for the next frame, allowing Kivy to complete the layout pass.
        Clock.schedule_once(lambda dt: self._wait_for_layout_and_fade(main_widget, duration), 0)

    def _wait_for_layout_and_fade(self, main_widget, duration):
        """
        This function is called after a short delay to ensure the UI is laid out.
        """
        try:
            ...
            def _cleanup(*_):
                ...
                try:
                    ...
                    pass
                # Mark fade as no longer active and clear any special abort flag
                self._explore_fade_active = False
                self.is_transitioning = False
                try:
                    # Do not assume App exists in all contexts
                    app = App.get_running_app()
                    if app and hasattr(app, 'tx_mgr') and app.tx_mgr is self:
                        # Only clear the abort_on_light_switch for the first-room case
                        self.abort_on_light_switch = False
                except Exception:
                    pass
            ...
            # On any failure, ensure overlay is not left behind
            try:
                if self.overlay and self.overlay.parent:
                    Window.remove_widget(self.overlay)
            except Exception:
    ...
    def slide_transition(self, direction: str, move_cb, duration=0.40):
        """
        Full-window slide: snapshot the entire root both before and after the move,
        then animate those full-frame images by Window.width/height. This covers
        the bands vertically (and horizontally), with zero stretching.
        """
        # --- Local imports so you don't need to change file headers ---
        from kivy.app import App
        from kivy.core.window import Window
        from kivy.uix.floatlayout import FloatLayout
        from kivy.uix.stencilview import StencilView
        ...
        # --- Snapshot import shim: try ui.background_snapshot, then background_snapshot,
        #     and finally a minimal fallback using export_as_image().
        try:
            from ui.background_snapshot import snapshot as snapshot
        except Exception:
            try:
                from background_snapshot import snapshot as snapshot
            except Exception:
                def snapshot(widget, downsample=1):
                    img = widget.export_as_image()
                    tex = img.texture
                    # mild smoothing to avoid aliasing if the driver scales
        ...
        # We'll hide these during the slide so they don't get double-rendered
        bg = getattr(game, "environment_loader", None)
        title = game.ids.get("title_label") if hasattr(game, "ids") else None
        details = game.ids.get("details_scroll") if hasattr(game, "ids") else None
        underline = game.ids.get("title_underline") if hasattr(game, "ids") else None

        def _hide_live_ui():
            if bg: bg.opacity = 0
            if title: title.opacity = 0
            if details: details.opacity = 0
            if underline: underline.opacity = 0

        def _restore_live_ui():
            if bg:
                bg.opacity = 1

            fade_dur = 0.16
            is_dark = bool(getattr(game, "_room_is_dark", False))
            ...
            if title:
                if is_dark:
                    ...
                    Animation(opacity=1, duration=fade_dur, t="in_out_quad").start(title)

            if underline:
                if is_dark:
                    underline.opacity = 0
                else:
                    Animation(opacity=1, duration=fade_dur, t="in_out_quad").start(underline)

            if details:
                Animation(opacity=1, duration=fade_dur, t="in_out_quad").start(details)

            try:
                if hasattr(game, "_apply_dark_ui_state"):
                    game._apply_dark_ui_state(is_dark)
            except Exception:
                pass

        # The snapshot target for a full-window capture
        root_widget = getattr(app, "transition_snapshot_root", None) or app.root
        if not root_widget:
            # nothing sensible to animate
            self.is_transitioning = False
            return

        # ------------------- Build full-window frames -------------------
        # Hide live UI *before* we capture the OLD frame
        _hide_live_ui()
        old_tex = snapshot(root_widget, downsample=1)
        try:
            old_tex.flip_vertical()
        except Exception:
            pass

        # Perform the move to the next room, then capture the NEW frame
        if callable(move_cb):
            try:
                move_cb()
            except Exception:
        ...
        new_tex = snapshot(root_widget, downsample=1)
        try:
            new_tex.flip_vertical()
        except Exception:
            pass

        # Full-window images (no scaling; they match Window.size 1:1)
        ww, wh = Window.size
        old_img = Image(texture=old_tex, size=(ww, wh), size_hint=(None, None),
                        pos=(0, 0), allow_stretch=False, keep_ratio=True, opacity=1.0)
        new_img = Image(texture=new_tex, size=(ww, wh), size_hint=(None, None),
                        pos=(0, 0), allow_stretch=False, keep_ratio=True, opacity=0.0)

        # ------------------- Overlay setup -------------------
        self.overlay = FloatLayout(size=(ww, wh), pos=(0, 0), opacity=0)
        # Stencil keeps everything clipped to the window bounds
        stencil = StencilView(size=(ww, wh), pos=(0, 0))
        self.overlay.add_widget(stencil)
        # Draw order: NEW below, OLD on top (so we "push" OLD away)
        stencil.add_widget(new_img)
        stencil.add_widget(old_img)
        ...
        # Make overlay visible next frame (lets Kivy lay it out first)
        Clock.schedule_once(lambda *_: setattr(self.overlay, "opacity", 1.0), 0)

        # ------------------- Animate by FULL WINDOW size -------------------
        if direction == "east":      # swipe left
            anim_old = Animation(x=old_img.x - ww, opacity=0.0, duration=duration, t="in_out_quad")
            Animation(x=0, duration=duration, t="in_out_quad").start(new_img)
        elif direction == "west":    # swipe right
            anim_old = Animation(x=old_img.x + ww, opacity=0.0, duration=duration, t="in_out_quad")
            Animation(x=0, duration=duration, t="in_out_quad").start(new_img)
        elif direction == "north":   # swipe down
            anim_old = Animation(y=old_img.y - wh, opacity=0.0, duration=duration, t="in_out_quad")
            Animation(y=0, duration=duration, t="in_out_quad").start(new_img)
        else:                        # "south" (swipe up)
            anim_old = Animation(y=old_img.y + wh, opacity=0.0, duration=duration, t="in_out_quad")
            Animation(y=0, duration=duration, t="in_out_quad").start(new_img)

        # Fade the new frame in as it slides into place
        Animation(opacity=1.0, duration=duration, t="in_out_quad").start(new_img)
        ...
        def _finish(*_args):
            ...
            try:
                if added_to == "window":
                    Window.remove_widget(self.overlay)
                elif added_to == "root" and getattr(app, "root", None) and self.overlay.parent:
                    app.root.remove_widget(self.overlay)
            except Exception:
                pass
            self.overlay = None
            self.is_transitioning = False

        anim_old.bind(on_complete=_finish)
        anim_old.start(old_img)
    ...
    def play_combined_combat_entry(self, switch_screen_callback, duration=1.8):
        ...
        battle_text.pos  = (Window.width, Window.height * 0.75)
        ...
        # --- THIS IS THE FIX ---
        # The text label's width isn't guaranteed to be correct until the next
        # frame. We defer the animation start to ensure its `center_x` is
        # calculated accurately, resulting in perfect centering every time.
        anim_text_in   = Animation(center_x=Window.width * 0.56, opacity=1, duration=p2 * .5, t='out_back')
        
        anim_fade_in_black = Animation(opacity=1, duration=p2)
        anim_slash     = (Animation(opacity=1, width=dp(100), duration=p2 * .2, t='out_cubic') +
        ...
        # MODIFIED: Animation for the battle text to slide into view. Nudged to the left.
        # --- Stage 4: Animation callbacks ---
        def _midpoint(*_):
            if switch_screen_callback:
                switch_screen_callback(composed_tex)
            
            # --- FIX: Hide speed lines instantly, then animate out other elements ---
            speed_lines_bg.opacity = 0
            anim_exit_rush.start(rush_sprite)
            (Animation(center_x=Window.width * 1.5, opacity=0, duration=p3, t='in_cubic')).start(battle_text) # Text now slides out with the sprite.
        ...
        # Schedule the text animation to start on the next frame.
        Clock.schedule_once(lambda _: anim_slash.start(slash_effect), p1 * 0.7)
        Clock.schedule_once(lambda _: anim_text_in.start(battle_text), p1) # This now runs after layout.
        # --- END OF FIX ---

    # ------------------------------------------------------------------ #
    # Exploration → System overlay
    # ------------------------------------------------------------------ #
