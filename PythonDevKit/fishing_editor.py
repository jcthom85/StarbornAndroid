...
class FishingEditor(QWidget):
    ...
    def _reload_lists(self):
        ...
        for s in self.data.get("rods", []):
            label = s if isinstance(s, str) else str(s.get("name") or s.get("id") or s)
            self.rod_list.addItem(label)
        for s in self.data.get("lures", []):
            label = s if isinstance(s, str) else str(s.get("name") or s.get("id") or s)
            self.lure_list.addItem(label)
