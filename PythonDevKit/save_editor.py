...
class SaveEditor(QMainWindow):
    def __init__(self, project_root: Optional[str] = None):
        ...
        self.assets_dir = paths.assets_dir
        self.saves_dir = self.assets_dir / "saves"
        if not self.saves_dir.exists():
            self.saves_dir = self.assets_dir
        ...
        self.items = _safe_load_json(self.assets_dir / "items.json", [])
        self.rooms = _safe_load_json(self.assets_dir / "rooms.json", [])
        self.skills = _safe_load_json(self.assets_dir / "skills.json", [])
        self.progression = _safe_load_json(self.assets_dir / "progression.json", {})
        self.level_curve = LevelCurve(self.assets_dir / "leveling_data.json")
    ...
    def _on_open(self):
        ...
        start_dir = str(self.saves_dir if self.saves_dir.exists() else self.assets_dir)
    ...
    def _on_save_as(self):
        ...
        default_dir = self.saves_dir if (self._enveloped and self.saves_dir.exists()) else self.assets_dir
