...
Files used: worlds.json, hubs.json, hub_nodes.json, rooms.json, room_templates.json
...
def _detect_project_root() -> str:
    return str(resolve_paths(Path(__file__).parent).assets_dir)
...
class WorldBuilder(QMainWindow):
    ...
    def _templates_path(self) -> str:
        return str(Path(__file__).resolve().parent / "templates.json")
    ...
    def _load_all_data(self):
        ...
        nodes = _safe_read_json(os.path.join(self.root, "hub_nodes.json"), [])
        rooms = _safe_read_json(os.path.join(self.root, "rooms.json"), [])
        self.worlds_by_id = _to_id_map(worlds, id_key='id')

        # Heuristic: assign missing world_id to a reasonable default (non-debug world if available)
        default_world_id = None
        for w in worlds:
            wid = w.get("id")
            if wid and "debug" not in str(wid).lower():
                default_world_id = wid
                break
        if default_world_id is None and worlds:
            default_world_id = worlds[0].get("id")

        hubs_normalized = []
        for h in hubs:
            if isinstance(h, dict) and not h.get("world_id"):
                h = dict(h)
                h["world_id"] = default_world_id
            hubs_normalized.append(h)
        hubs = hubs_normalized

        nodes_normalized = []
        for n in nodes:
            if isinstance(n, dict) and not n.get("world_id"):
                n = dict(n)
                # inherit from hub if present; else default
                hub_id = n.get("hub_id")
                hub_world = next((h.get("world_id") for h in hubs if isinstance(h, dict) and h.get("id") == hub_id and h.get("world_id")), None)
                n["world_id"] = hub_world or default_world_id
            nodes_normalized.append(n)
    ...
    def _save_all_data(self, *, show_toast: bool = True):
        ...
        if not _safe_write_json(os.path.join(self.root, "hub_nodes.json"),  sorted(self.nodes_by_id.values(),  key=lambda x: x.get("id","")), "Nodes"): ok=False
    ...
    def _rebuild_tree(self):
        ...
        for wid, w in sorted(self.worlds_by_id.items()):
            ...
            for hid, h in sorted(self.hubs_by_id.items()):
                if h.get('world_id') not in (wid, None, ""):
                    continue
