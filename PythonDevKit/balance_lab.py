...
PATHS = resolve_paths(Path(__file__).parent)
ASSETS_ROOT = str(PATHS.assets_dir)
PROJECT_ROOT = str(PATHS.project_root)
DEF_CFG = str(Path(__file__).parent / "balance_targets.json")
...
class BalanceConfig:
    ...
    def path(self, key: str) -> str:
        ...
        if not rel:
            return ""
        base = PROJECT_ROOT if key == "reports_dir" else ASSETS_ROOT
...
class Reporter:
    def __init__(self, cfg: BalanceConfig):
        self.cfg = cfg
        self.out_dir = os.path.join(PROJECT_ROOT, self.cfg.paths.get("reports_dir") or "reports")
