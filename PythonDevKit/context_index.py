...
from devkit_paths import resolve_paths, DevkitPaths
...
class ContextIndex:
    def __init__(self, project_root: Optional[str|Path] = None):
        self.paths: DevkitPaths = resolve_paths(Path(project_root) if project_root else Path(__file__).parent)
        self.project_root = self.paths.project_root
        self.assets_root = self.paths.assets_dir
        self.devkit_root = Path(__file__).resolve().parent
        self.root = self.assets_root  # backward compat for callers expecting .root to point at JSON
    ...
    def nodes(self) -> list:
        return _read_list(self.root / "hub_nodes.json")
    ...
    def pack_global(self):
        ...
        def _abs(p: os.PathLike | str | Path) -> Path:
            ...
            if p.is_absolute():
                return p
            for base in (self.assets_root, self.project_root, self.devkit_root):
                if not base:
                    continue
                candidate = base / p
                if candidate.exists():
                    return candidate
            return self.assets_root / p
        ...
        return {
            ...
            "nodes": _read_list_safe("hub_nodes.json"),
            ...
            # Tools data
            "templates": _read_dict("templates.json"),
            "balance_targets": _read_dict("balance_targets.json"),
